...
async def test_delete_job_cascades_its_runs_and_ticket(
    ...
    from conductor import agent_runs, job_events
    from conductor.jobs import delete_job
    from conductor.models import AgentRunLog, JobEvent, Ticket
    ...
    # Both jobs share ticket T-1, but their runs + events are scoped by job_id.
    for jid, out in ((job1_id, "a"), (job2_id, "b")):
        ...
        await job_events.record_event(
            sessionmaker, job_id=jid, message="opened PR", ticket_id="T-1"
        )
    ...
    async with sessionmaker() as session:
        ...
        runs = (await session.execute(select(AgentRunLog))).scalars().all()
        events = (await session.execute(select(JobEvent))).scalars().all()
        # Only job1's records are gone; the re-trigger's survive despite the shared ticket_id.
        assert [r.job_id for r in runs] == [job2_id]
        assert [e.job_id for e in events] == [job2_id]
